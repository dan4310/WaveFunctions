...
public class Listener implements ActionListener {
	...
	public Listener(JComboBox<String>[] cb, JTextField[] k, JTextField[] A, JCheckBox check) {
		comboBoxes = cb; kTextFields = k; ATextFields = A; this.check = check;
	}
	public Listener() {
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (b.getText().equals("Plot!")) {
			...
			for (int i = 0; i < comboBoxes.length; i++) {
				...
				} catch (Exception e2) { k[i] = 0; A[i] = 0;}
				
			}
			if (total > 0) {
				if (check.isSelected()) {
					gNew = new Graph(funcInts, k, A, true);
				} else { gNew = new Graph(funcInts, k, A); }
